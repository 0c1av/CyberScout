	...
	try:
		...
		if response.status_code == 200:
			...
			if proxy_list:
				...
				print(f"{ORANGE}[WARNING] No proxies returned by the API.{RESET}")
				return None
		else:
			print(f"[ERROR] Failed to fetch proxies from the API.{RESET}")
			return None
	except requests.exceptions.RequestException as e:
		print(f"{RED}[ERROR] Unable to fetch proxy: {e}{RESET}")
		return None
